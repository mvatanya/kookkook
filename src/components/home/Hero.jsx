import { motion } from "framer-motion";
import Button from "../ui/Button";

function Hero() {
  return (
    <section className="relative pt-24 pb-16 overflow-hidden bg-secondary">
      <div className="absolute top-0 left-0 w-full h-full bg-secondary z-0">
        {/* Abstract decorative elements */}
        <div className="absolute -right-12 top-20 w-48 h-48 bg-primary opacity-20 decorative-shape"></div>
        <div className="absolute left-16 bottom-20 w-32 h-32 bg-primary opacity-10 decorative-shape"></div>
        <div className="absolute right-1/4 top-1/3 w-24 h-24 bg-primary opacity-15 decorative-shape"></div>
        <div className="absolute left-1/3 top-1/4 w-40 h-40 bg-primary opacity-10 decorative-shape"></div>
        <div className="absolute right-1/3 bottom-1/4 w-36 h-36 bg-primary/20 decorative-shape"></div>
      </div>

      <div className="container mx-auto px-4 pt-10 pb-16 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <motion.div
            initial={{ opacity: 0, x: -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
            className="text-left"
          >
            <h1 className="text-5xl md:text-6xl font-bold text-white mb-6">
              <span className="text-primary">Spice Up</span> Your Life
            </h1>

            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: 0.3, duration: 0.8 }}
            >
              <p className="text-xl md:text-2xl font-medium text-white mb-2">
                Experience the true taste of Thailand
              </p>
              <p className="text-lg text-white/80 mb-8">
                Crafted sriracha, inspired by the soul of our heritage
              </p>
            </motion.div>

            <div className="flex flex-col sm:flex-row gap-4">
              <Button primary>Explore Products</Button>
              <Button secondary>Find Stores</Button>
            </div>
          </motion.div>

          <motion.div
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
            className="hero-image-container flex justify-center lg:justify-end relative"
          >
            <img
              src="https://static.wixstatic.com/media/15bb16_ff778dbfa8c54cbd9f43e973c593b6f3~mv2.png/v1/crop/x_0,y_86,w_1146,h_2876/fill/w_558,h_1410,al_c,q_90,usm_0.66_1.00_0.01,enc_avif,quality_auto/Super%20hot%20kook%20kook%20525g.png"
              alt="Kookkook Sriracha Bottle"
              className="hero-image max-h-[520px] w-auto"
            />
          </motion.div>
        </div>
      </div>

      {/* Wave/diagonal separator */}
      <div
        className="absolute bottom-0 left-0 right-0 h-20 bg-white"
        style={{ clipPath: "polygon(0 100%, 100% 0, 100% 100%)" }}
      ></div>
    </section>
  );
}

export default Hero;
